...
** Silence per-file auto-save recovery prompts
Emacs walks every stale ~#...#~ file on startup and prompts per-file in the
minibuffer, blocking startup for ~30s. We want the files left in place (crash
recovery via =M-x recover-this-file= on demand) but no per-file prompt.
#+begin_src emacs-lisp :tangle yes
